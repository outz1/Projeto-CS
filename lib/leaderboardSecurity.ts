export type GameType = "snake" | "arcade";
...
const ARCADE_SCORE_MAX = 500_000;
...
function stripUnsafeText(value: string): string {
  return value.replace(CONTROL_CHARS_REGEX, "").replace(INVISIBLE_CHARS_REGEX, "").replace(BIDI_CHARS_REGEX, "");
}
...
export function sanitizePlayerNameInput(raw: unknown): string {
  ...
  const safe = stripUnsafeText(normalized).replace(UNSAFE_NAME_CHARS_REGEX, "").replace(MULTI_SPACE_REGEX, " ").trim();
...
export function isValidGame(raw: unknown): raw is GameType {
  return raw === "snake" || raw === "arcade";
}

export function isValidScore(raw: unknown, game: GameType = "snake"): raw is number {
  if (typeof raw !== "number" || !Number.isInteger(raw) || raw < SCORE_MIN) return false;
  if (game === "snake") return raw <= SNAKE_SCORE_MAX && raw % SNAKE_SCORE_STEP === 0;
  if (game === "arcade") return raw <= ARCADE_SCORE_MAX;
  return false;
}

export function isValidDurationMs(raw: unknown): raw is number {
  return typeof raw === "number" && Number.isInteger(raw) && raw >= 0 && raw <= MAX_GAME_DURATION_MS;
}
...
export function parseScoreEntry(value: unknown): ScoreEntry | null {
  ...
  const game = isValidGame(gameRaw) ? gameRaw : "snake";
  const score = obj.score;

  if (!name || !isValidPlayerId(id) || !isValidScore(score, game)) return null;
...
export function parseScoreSubmitPayload(value: unknown): ScoreSubmitPayload | null {
  ...
  if (!normalizedName || !isValidPlayerId(id) || !isValidScore(score, "snake") || !isValidDurationMs(durationMs)) {
    return null;
  }

  if (isSuspiciousDuration(score, durationMs, "snake")) {
    return null;
  }
...
export function parseScoresApiResponse(payload: unknown): ScoreEntry[] {
  if (Array.isArray(payload)) {
    return payload.map(parseScoreEntry).filter((entry): entry is ScoreEntry => entry !== null).sort((a, b) => b.score - a.score);
  }

  const obj = asObject(payload);
  if (!obj || obj.ok !== true || !Array.isArray(obj.data)) return [];

  return obj.data.map(parseScoreEntry).filter((entry): entry is ScoreEntry => entry !== null).sort((a, b) => b.score - a.score);
}
