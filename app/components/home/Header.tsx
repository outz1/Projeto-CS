    ...
    <>
      ...
      >
        <div className="mb-8 pt-10">
          <span className="text-sm font-semibold uppercase tracking-wide text-[#0b1d4d]">Menu</span>
        </div>
        <nav className="flex flex-col gap-4 text-sm font-semibold text-[#0b1d4d]">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-md border border-[#8eb1ff]/60 bg-[#e4eeff]/70 px-3 py-3 break-words transition-colors hover:bg-[#f1f6ff]"
              onClick={() => setIsMenuOpen(false)}
            >
              {item.label}
            </a>
